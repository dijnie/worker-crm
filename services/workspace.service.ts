...
export class WorkspaceService {
  ...
  async get(): Promise<WorkspaceSettings> {
    ...
    if (!row) throw new ServiceError(404, "The workspace is not initialised");
  ...
  async update(inputActorId: string, input: unknown): Promise<WorkspaceSettings> {
    ...
    if (!actors.length) throw new ServiceError(403, "Only active system accounts can change workspace settings");
    ...
    const [updated] = await this.db.update(singletonWorkspace)
      ...
      .returning(fields);
    if (!updated) throw new ServiceError(409, "The workspace settings changed; reload and try again");
