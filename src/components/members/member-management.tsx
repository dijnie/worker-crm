    ...
    <div className="p-4 md:p-6">
      <div className="mx-auto flex w-full min-w-0 max-w-7xl flex-col gap-6">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground">Everyone with active access shares the CRM workspace. Owners can manage roles and access. At least one active owner must remain.</p>
        </div>
            ...
            {data.items.map((member) => <li key={member.id} className="flex min-w-0 flex-col gap-4 p-4 lg:flex-row lg:items-center lg:p-6">
              ...
              <div className="flex flex-wrap items-center gap-2 lg:w-64 lg:shrink-0 lg:justify-end">
                {member.status === "active" ? <>
                  <Button variant="outline" className="min-h-11" disabled={disabled} aria-label={`Make ${member.name} ${member.role === "owner" ? "a member" : "an owner"}`} onClick={() => mutate(member, { action: "change-role", role: member.role === "owner" ? "member" : "owner", expectedRevision: member.revision })}>
                    {pendingId === member.id ? "Saving…" : member.role === "owner" ? "Make member" : "Make owner"}
                  </Button>
      ...
      <Dialog open={revokeTarget !== null} onOpenChange={(open) => { if (!open && !pendingId) { setRevokeTarget(null); setActionError(""); } }}>
        <DialogContent className="w-[calc(100%_-_2rem)] rounded-lg [&>button:last-child]:size-11 [&>button:last-child]:right-1 [&>button:last-child]:top-1 [&>button:last-child]:flex [&>button:last-child]:items-center [&>button:last-child]:justify-center" onEscapeKeyDown={(event) => { if (pendingId) event.preventDefault(); }} onPointerDownOutside={(event) => { if (pendingId) event.preventDefault(); }}>
          ...
          <DialogDescription className="break-words leading-relaxed">{revokeTarget?.name} will be signed out and lose access to the shared workspace. Their CRM records and history will remain. An owner can restore access later.</DialogDescription>
          {revokeTarget?.id === currentUserId && <p className="text-sm">You are revoking your own access. Another owner will need to restore it.</p>}
