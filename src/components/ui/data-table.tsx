...
import type { TableSelection } from "@/hooks/use-table-selection";
import { ROW_ACCENT, ROW_ACCENT_EXPANDABLE } from "@/lib/ui/row-accent";
...
export function DataTable<TRow, TSub = unknown>({
	...
	const [hidden, setHidden] = useState<string[]>(defaultHiddenIds);
	const [restoredColumns, setRestoredColumns] = useState(false);
	...
	useEffect(() => {
		...
		setRestoredColumns(true);
	}, [storageKey, defaultHiddenIds]);

	useEffect(() => {
		if (!storageKey || !restoredColumns) return;
		try {
			window.localStorage.setItem(storageKey, JSON.stringify(hidden));
		} catch {
			// A full or unavailable store keeps the in-memory choice for this session.
		}
	}, [hidden, storageKey, restoredColumns]);
						...
						{hideable.length > 0 && (
							<DropdownMenu>
								...
								<DropdownMenuContent align="end" className="min-w-48">
									...
									<DropdownMenuItem
										disabled={
											hidden.length === defaultHiddenIds.length &&
											hidden.every((id) => defaultHiddenIds.includes(id))
										}
