    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[calc(100%_-_2rem)] max-w-sm rounded-lg [&>button:last-child]:size-11 [&>button:last-child]:right-1 [&>button:last-child]:top-1 [&>button:last-child]:flex [&>button:last-child]:items-center [&>button:last-child]:justify-center">
        ...
        <dl className="space-y-3 text-sm">
          ...
          <div><dt className="text-muted-foreground">Role</dt><dd>{account.role === "owner" ? "Owner" : "Member"}</dd></div>
        </dl>
        {account.role === "owner" && <Button asChild variant="outline" className="min-h-11"><Link href="/settings/members" onClick={() => setOpen(false)}>Manage members</Link></Button>}
        <Button variant="outline" className="min-h-11" disabled={pending} onClick={signOut}>{pending ? "Signing out…" : "Sign out"}</Button>
