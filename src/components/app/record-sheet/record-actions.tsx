...
import { RECORD_INVALIDATIONS, recordEntityKey, type RecordEntity } from "../records/form-values";
...
export function RecordActions({ entity, id, archivedAt }: { entity: RecordEntity; id: string; archivedAt: string | null }) {
  ...
  const { actions: copy, common } = dictionary.recordSheet;
  ...
  async function run() {
    ...
    try {
      ...
      setConfirm(false); setMessage(archivedAt ? copy.restored : copy.archived);
  ...
  return <div className="space-y-2">
    <div className="flex items-center gap-2">
      {archivedAt && <span className="rounded-sm bg-muted px-2 py-0.5 text-muted-foreground text-xs">{common.archived}</span>}
      <Button type="button" variant="outline-ghost" size="sm" disabled={pending} onClick={() => archivedAt ? void run() : setConfirm(true)}>{pending ? copy.updating : archivedAt ? copy.restoreRecord : copy.archiveRecord}</Button>
    </div>
    ...
    <Dialog open={confirm} onOpenChange={open => { if (!pending) setConfirm(open); }}><DialogContent className="gap-3 sm:max-w-md">
      <DialogTitle>{copy.archiveTitle(dictionary.crm.entities[recordEntityKey(entity)].lower)}</DialogTitle><DialogDescription>{copy.archiveDescription}</DialogDescription>
      {error && <p role="alert" className="text-destructive text-xs">{error}</p>}
      <div className="flex justify-end gap-2"><Button data-dialog-close variant="outline" disabled={pending} onClick={() => setConfirm(false)}>{dictionary.common.cancel}</Button><Button disabled={pending} onClick={() => void run()}>{pending ? copy.archiving : dictionary.recordSheet.bulk.archive}</Button></div>
