...
export function TimelineEntry({ activity, now, directory, labels, record, onResult }: TimelineEntryProps) {
  ...
  const links = activityRecordLinks(activity).filter(link => link.kind !== record.kind || link.id !== record.id);
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1 space-y-2">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <h4 className="break-words text-sm font-medium">{activity.subject || presentation.label}</h4>
